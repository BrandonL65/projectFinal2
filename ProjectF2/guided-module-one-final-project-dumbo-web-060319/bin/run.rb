...
def sayHi
    ...
    res = gets.chomp 
    progressBar
    puts "Hello, #{res}, are you ready to play? (Y/N)"
    res2 = gets.chomp 
    progressBar
    arrOfAns = ["Aw why not, cmon try it! (Y/N)", "Who knows, maybe you'll love politics! Try! (Y/N)", "The first step to knowledge is to guess. (Y/N)", "Wall is Building. Press Y to stop. (Y/N)"]
    while (res2 != "Y")
        ran = arrOfAns.sample 
        puts ran 
        res2 = gets.chomp
